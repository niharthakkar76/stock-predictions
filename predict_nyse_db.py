...
def predict_stock(symbol, model):
    ...
    try:
        ...
        prediction_data = {
            ...
            'rating': rating,
            'expected_return': max(min(float(expected_return), 99.99), -99.99),  # Ensure within decimal(5,2) range
        ...
        result = supabase.table('nyse_stock_data').insert(prediction_data).execute()
...
if __name__ == "__main__":
    ...
    print("\nLoading NYSE symbols...")
    nyse_df = pd.read_csv('nse.csv')
    test_stocks = nyse_df['Symbol'].tolist()
    print(f"Loaded {len(test_stocks)} NYSE symbols")
